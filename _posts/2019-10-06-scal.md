...
## Conclusions
Both the theory that motivated this and the implementation could have been simpler. First of all, it makes sense that a forced vibration has a unique linear-modal signature. In that sense, any not completely symmetric structure can be used as a structural/electrical (or "electromechanical") compressed sensing circuit. The key is incoherent measurements and a downstream algorithm to make sense of the incoming data. I believe deep learning has a lot to offer on applications of this sort.

It's difficult to model exactly how the structure is excited, but one does not really need to model exactly how it is excited to perform the down-stream task (2D localization).
