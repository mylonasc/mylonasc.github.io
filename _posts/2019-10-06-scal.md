...
An aditional motivation for this project is that I'm working with dilated DenseNets lately. The DenseNet is a particular type of convolutional neural networks with shortcut connections from all layers to all layers. I wanted a controlled non-trivial dataset to test a DenseNet with an architecture similar to the one I'm using for the other problem. 
...
Therefore the compressive sensing circuit is the sensor itself. The focus of compressive sensing seems to be on digital random integrators - the approach I used is an alternative and much simpler one.
...
## Running the model
A python script was written to evaluate the model in real-time. The model actually does perform quite well - with an accuracy approximately 75% in real-time. Further improvements would be possible with a better model and of course more data.

### Scaling to IKEA table
I have implemented the same method to an IKEA table. The network performed relatively well - also in real-time.
...
What is different in this application is that instead of making incoherent measurements I make a sensing circuit that is incoherent with the measurement that I want to perform - namely it is excited in a unique way depending on where I excite it. It's difficult to model exactly how it is excited, but one does not really need to model exactly how it is excited to perform 2D localization. That is why deep learning is used for that. Actually even it is not obvious from the results here, PCA gives quite decent results. It is worth trying an simple softmax classifier with the PCA components.
