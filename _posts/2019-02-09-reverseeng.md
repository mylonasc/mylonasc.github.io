...
# The project(s)
I came across an infra-red (IR) controlled stereo and I wanted to make it remote controlled from my PC. Although apparently this is totally unnecessary, it is a potentially 
useful project since IR is everywhere. 
...
Actually after writing this post, I realized that if I set the shutter speed very high on my samsung S7, I can clearly see the binary code pop out! Here is a picture from the sony remote I managed to get signals such as the following one
...
Luckily, the borders of the patterns are quite crisp.
...
This worked sort-of ok, but with some small additional caveats I detail bellow. Firstly, the LED light does not reach the whole screen with the same intensity. 
...
I then continued tackling the missing frames issue. I needed a way to somehow align the signals.I was hoping it should be possible to fill-in the blanks by summing/averaging. 
...
However, a different signal is sent every time a button is pressed for the on-off command. This is the first command I needed to reverse engineer since otherwise I would need to have the
...
Fortunately it is simple to separate these signals, especially if one considers that they are strictly positive and there are exactly two of them (i.e. the on and off command). 
I first tried some hand-designed tricks (like figuring out visually what bit belongs to just one signal etc) but decided to put some simple ML to use to do this more effectively.
