...
No post on generative models would be complete with some fancy interpolations!
Since this is a Conditional VAE, and I know beforehand the influencing factors I care about (I just don't want to model their effect explicitly or manually) I'm not doing latent space interpolations but I'm simply changing the conditioning variables. 
The latent space does have an effect and it is seen on the previous plots. Namely its effect is observed in the variability on the sampled estimates of damage equivallent loads.
